public class Rope {
  public Vec2 top;
  public Vec2 pos[], vel[], acc[];
  public float restLen;
  
  public Rope(Vec2 top) {
    this.top = top;
    this.pos = new Vec2[numNodes];
    this.vel = new Vec2[numNodes];
    this.acc = new Vec2[numNodes];
    
    float xChange = random(2, 4);
  ...
  public void updateSelf(float dt) {
    ...
    for (int i = 0; i < numNodes; i++){
      ...
      Vec2 distance = pos[i].minus(obstaclePosition);
